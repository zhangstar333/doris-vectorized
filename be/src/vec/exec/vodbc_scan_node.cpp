    ...
    do {
        ...
        for (int row_index = 0; true; row_index++) {
            ...
            for (int column_index = 0, materialized_column_index = 0; column_index < column_size;
                ...
                bool sucees = text_converter->write_column(
                        slot_desc, &columns[column_index],
                        static_cast<char*>(column_data.target_value_ptr), column_data.strlen_or_ind,
                        true, false);
                DCHECK(sucees) << ":converter fail!";
